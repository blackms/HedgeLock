...
class AccountData(BaseModel):
    ...
    def ltv(self) -> float:
        ...
        if self.total_collateral_value == 0:
            return 0.0
        return (
            self.total_loan_value + self.total_interest
...
class RiskCalculation(BaseModel):
    ...
    # Funding context
    funding_context: Optional[FundingContext] = Field(
        default=None, description="Current funding context"
    )
    funding_adjusted_score: Optional[float] = Field(
        default=None, description="Risk score adjusted for funding"
    )

    # Processing metadata
    processing_time_ms: float = Field(description="Processing time in milliseconds")
    trace_id: Optional[str] = Field(
        default=None, description="Trace ID for correlation"
    )
...
class RiskStateMessage(BaseModel):
    ...
    funding_regime: Optional[FundingRegime] = Field(
        default=None, description="Current funding regime"
    )
    position_multiplier: Optional[float] = Field(
        default=None, description="Funding-based position multiplier"
    )
