...
class HedgeDecision(BaseModel):
    ...
    # Funding context
    funding_adjusted: bool = Field(
        default=False, description="Whether hedge was adjusted for funding"
    )
    funding_regime: Optional[FundingRegime] = None
    position_multiplier: Optional[float] = Field(
        default=1.0, description="Funding-based position multiplier"
    )
...
class OrderRequest(BaseModel):
    ...
    orderLinkId: Optional[str] = Field(
        default=None, description="User-defined order ID"
    )
...
class HedgeTradeMessage(BaseModel):
    ...
    funding_cost_24h: Optional[float] = Field(
        default=None, description="Projected 24h funding cost in USD"
    )
