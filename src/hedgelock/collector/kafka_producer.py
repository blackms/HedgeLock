...
class KafkaMessageProducer:
    ...
    async def publish_account_update(
        self, account_update: AccountUpdate, trace_id: Optional[str] = None
    ) -> bool:
            ...
            try:
                ...
                logger.info(
                    ...
                    collateral_value=float(
                        account_update.collateral_info.collateral_value
                    ),
    ...
    async def publish_market_data(
        self, market_data: MarketData, trace_id: Optional[str] = None
    ) -> bool:
        ...
        with trace_context(trace_id) as tid:
            try:
                ...
                await self.producer.send_and_wait(
                    topic, value=message, key=market_data.symbol.encode("utf-8")
                )
    ...
    async def publish_order_update(
        self, order_update: OrderUpdate, trace_id: Optional[str] = None
    ) -> bool:
        ...
        with trace_context(trace_id) as tid:
            try:
                ...
                await self.producer.send_and_wait(
                    topic, value=message, key=order_update.order_id.encode("utf-8")
                )
    ...
    async def publish_funding_rate(
        self, funding_message: FundingRateMessage, trace_id: Optional[str] = None
    ) -> bool:
