...
from .models import AccountUpdate, Balance, MarketData, OrderUpdate, Position
...
class CollectorService:
    ...
    async def _handle_market_data(self, market_data: MarketData) -> None:
        ...
        with trace_context() as trace_id:
            success = await self.kafka_producer.publish_market_data(
                market_data, trace_id
            )
            if success:
                ...
                with trace_context() as trace_id:
                    ...
                    if not self.current_balances:
                        self.current_balances = (
                            await self.rest_client.get_wallet_balance()
                        )
                    ...
                    # Publish to Kafka
                    success = await self.kafka_producer.publish_account_update(
                        account_update, trace_id
                    )
                    if success:
                ...
                with trace_context() as trace_id:
                    ...
                    if current_funding:
                        ...
                        message = FundingRateMessage(
                            funding_rate=funding_rate, trace_id=trace_id
                        )

                        # Publish to Kafka
                        success = await self.kafka_producer.publish_funding_rate(
                            message, trace_id
                        )
                        if success:
                            logger.info(
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        ws_connected = (
            self.ws_client.public_ws is not None and not self.ws_client.public_ws.closed
        )
