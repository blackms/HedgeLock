...
from .models import Position, Balance, MarketData, OrderUpdate, PositionSide, OrderStatus
...
class BybitWebSocketClient:
    ...
    async def _authenticate(self, ws: WebSocketClientProtocol) -> bool:
        ...
        try:
            ...
            if data.get("success") or (data.get("op") == "auth" and data.get("ret_msg") == "OK"):
        ...
        if topic.startswith("orderbook") or topic.startswith("trade"):
            ...
            if self.on_market_data and "data" in data:
                for item in data["data"]:
                    market_data = MarketData(
                        ...
                        bid=Decimal(str(item.get("b", [0])[0] if isinstance(item.get("b"), list) else item.get("b", 0))),
                        ask=Decimal(str(item.get("a", [0])[0] if isinstance(item.get("a"), list) else item.get("a", 0)))
                    )
            ...
            if self.on_market_data and "data" in data:
                ...
                market_data = MarketData(
                    ...
                    open_interest=Decimal(str(item.get("openInterest", 0))) if item.get("openInterest") else None,
                    funding_rate=Decimal(str(item.get("fundingRate", 0))) if item.get("fundingRate") else None
                )
                await self.on_market_data(market_data)
            ...
            if self.on_position_update and "data" in data:
                ...
                for pos_data in data["data"]:
                    position = Position(
                        ...
                        cum_realized_pnl=Decimal(str(pos_data.get("cumRealisedPnl", 0))),
                        position_value=Decimal(str(pos_data.get("positionValue", 0))),
                        leverage=Decimal(str(pos_data.get("leverage", 1))),
                        updated_time=datetime.fromtimestamp(int(pos_data.get("updatedTime", 0)) / 1000)
                    )
            ...
            if self.on_order_update and "data" in data:
                for order_data in data["data"]:
                    order = OrderUpdate(
                        ...
                        created_time=datetime.fromtimestamp(int(order_data.get("createdTime", 0)) / 1000),
                        updated_time=datetime.fromtimestamp(int(order_data.get("updatedTime", 0)) / 1000)
                    )
                    await self.on_order_update(order)
    
    async def _handle_message(self, ws: WebSocketClientProtocol, is_private: bool) -> None:
            ...
            async for message in ws:
                try:
                    ...
                    if data.get("op") == "subscribe":
                        if data.get("success"):
                            logger.info(f"Successfully subscribed to: {data.get('ret_msg')}")
                        else:
    ...
    async def _connect_public(self) -> None:
        ...
        while self.is_running:
            try:
                logger.info(f"Connecting to public WebSocket: {self.config.ws_public_url}")
            ...
            if self.is_running:
                logger.info(f"Reconnecting public WebSocket in {self.reconnect_delay} seconds...")
    ...
    async def _connect_private(self) -> None:
        ...
        while self.is_running:
            try:
                logger.info(f"Connecting to private WebSocket: {self.config.ws_private_url}")
            ...
            if self.is_running:
                logger.info(f"Reconnecting private WebSocket in {self.reconnect_delay} seconds...")
                await asyncio.sleep(self.reconnect_delay)
                self.reconnect_delay = min(self.reconnect_delay * 2, self.max_reconnect_delay)
