...
from ..shared.kafka_base import KafkaProducerBase, KafkaConsumerBase
from ..shared.funding_models import FundingRateMessage, FundingContextMessage
from .config import FundingEngineConfig

logger = logging.getLogger(__name__)


class FundingEngineProducer(KafkaProducerBase):
    ...
    async def send_funding_context(self, message: FundingContextMessage):
        ...
        await self.producer.send(
            self.config.funding_context_topic,
            value=message.dict()
        )
        logger.debug(f"Sent funding context for {message.funding_context.symbol}")


class FundingEngineConsumer(KafkaConsumerBase):
