...
from ..shared.funding_models import FundingRate, FundingSnapshot
...
async def lifespan(app: FastAPI):
    ...
    logger.info(f"FundingEngineConfig loaded: kafka_bootstrap_servers={config.kafka_bootstrap_servers}")
...
async def get_funding_history(symbol: str, hours: int = 24):
    ...
    if hours > 168:  # Max 7 days
        raise HTTPException(
            status_code=400, detail="Maximum 168 hours (7 days) allowed"
        )
...
async def simulate_funding_regime(request: SimulateRequest):
    """Simulate funding regime detection with custom rates."""
    try:
        # Create mock funding rates
        current_funding = FundingRate(
            symbol=request.symbol,
            funding_rate=request.current_rate / 100 / 365 / 3,  # Convert from APR
        ...
        mock_24h = []
        for i, rate in enumerate(request.rates_24h):
            mock_24h.append(
                FundingRate(
                    symbol=request.symbol,
        ...
        mock_7d = []
        for i, rate in enumerate(request.rates_7d):
            mock_7d.append(
                FundingRate(
                    symbol=request.symbol,
                    funding_rate=rate / 100 / 365 / 3,
