...
class FundingStorage:
    ...
    def store_funding_rate(self, rate: FundingRate):
        ...
        cutoff_time = datetime.utcnow() - timedelta(
            hours=self.config.history_window_hours
        )
        self.redis_client.zremrangebyscore(key, 0, cutoff_time.timestamp())

        # Update current rate
        current_key = f"funding:current:{rate.symbol}"
        self.redis_client.setex(current_key, 3600, value)  # 1 hour TTL

        logger.debug(
            f"Stored funding rate for {rate.symbol}: {rate.annualized_rate:.2f}% APR"
        )

    def get_funding_history(self, symbol: str, hours: int = 24) -> List[FundingRate]:
    ...
    def store_funding_regime(self, symbol: str, regime: FundingRegime):
        ...
        self.redis_client.setex(key, 3600, regime.value)  # 1 hour TTL
