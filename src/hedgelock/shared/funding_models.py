...
class FundingContext(BaseModel):
    ...
    position_multiplier: float = Field(
        description="Position size multiplier (0.0-1.0)", ge=0.0, le=1.0
    )
    should_exit: bool = Field(default=False, description="Emergency exit flag")
    regime_change: bool = Field(
        default=False, description="Regime changed from previous"
    )

    # Cost projections
    daily_cost_bps: float = Field(description="Projected daily cost in basis points")
    weekly_cost_pct: float = Field(description="Projected weekly cost as percentage")
...
class FundingDecision(BaseModel):
    ...
    funding_risk_score: float = Field(
        description="Funding risk score 0-100", ge=0, le=100
    )
