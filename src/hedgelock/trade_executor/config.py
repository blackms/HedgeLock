...
class KafkaConfig(BaseSettings):
    ...
    consumer_group: str = Field(
        default="trade-executor-group",
        env="KAFKA_CONSUMER_GROUP"
    )
    topic_hedge_trades: str = Field(
        default="hedge_trades",
        env="KAFKA_TOPIC_HEDGE_TRADES"
    )
    topic_trade_confirmations: str = Field(
        default="trade_confirmations",
        env="KAFKA_TOPIC_TRADE_CONFIRMATIONS"
    )
...
class BybitConfig(BaseSettings):
    ...
    max_orders_per_second: int = Field(
        default=10,
        env="BYBIT_MAX_ORDERS_PER_SECOND"
    )
    
    # Order defaults
    time_in_force: str = Field(
        default="IOC",
        env="BYBIT_TIME_IN_FORCE"
    )
    position_idx: int = Field(
        default=0,
        env="BYBIT_POSITION_IDX"
    )
...
class ExecutorConfig(BaseSettings):
    ...
    max_retries: int = Field(
        default=3,
        env="EXECUTOR_MAX_RETRIES"
    )
    retry_delay_ms: int = Field(
        default=1000,
        env="EXECUTOR_RETRY_DELAY_MS"
    )
    
    # Safety limits
    max_order_size_btc: float = Field(
        default=1.0,
        env="EXECUTOR_MAX_ORDER_SIZE_BTC"
    )
    max_daily_volume_btc: float = Field(
        default=10.0,
        env="EXECUTOR_MAX_DAILY_VOLUME_BTC"
    )
    
    # Performance
    order_timeout_ms: int = Field(
        default=5000,
        env="EXECUTOR_ORDER_TIMEOUT_MS"
    )
    status_poll_interval_ms: int = Field(
        default=500,
        env="EXECUTOR_STATUS_POLL_INTERVAL_MS"
    )
    
    # Monitoring
    metrics_port: int = Field(
        default=9094,
        env="EXECUTOR_METRICS_PORT"
    )
    health_check_interval_s: int = Field(
        default=30,
        env="EXECUTOR_HEALTH_CHECK_INTERVAL_S"
    )
