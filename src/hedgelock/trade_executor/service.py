...
class TradeExecutorService:
            ...
            try:
                ...
                if message:
                    await self._process_hedge_trade(
                        message["value"], message.get("trace_id")
                    )
    ...
    async def _process_hedge_trade(
        self, hedge_trade: Dict[str, Any], trace_id: Optional[str] = None
    ) -> None:
        ...
        try:
            ...
            await self.kafka.publish_confirmation(confirmation.dict(), trace_id)
            ...
            confirmation = TradeConfirmation(
                ...
                total_latency_ms=(datetime.utcnow() - start_time).total_seconds()
                * 1000,
                risk_state=hedge_trade["risk_state"],
                ltv=hedge_trade["ltv"],
                hedge_size=hedge_trade["hedge_decision"]["hedge_size"],
    ...
    async def _check_safety_limits(self, execution: TradeExecution) -> None:
        ...
        if (
            self.daily_volume + execution.quantity
            > self.config.executor.max_daily_volume_btc
        ):
            raise ValueError(
    ...
    async def _track_order_status(self, execution: TradeExecution) -> None:
        ...
        while (datetime.utcnow() - start_time).total_seconds() < timeout:
            ...
            order_update = await self.bybit.get_order_status(
                symbol=execution.symbol, order_link_id=execution.order_link_id
            )

            if not order_update:
                await asyncio.sleep(self.config.executor.status_poll_interval_ms / 1000)
                continue
        ...
        if execution.status == ExecutionStatus.SUBMITTED:
            execution.status = ExecutionStatus.FAILED
            execution.error = ExecutionError(
                code="ORDER_TIMEOUT", message="Order tracking timeout"
            )

    def _update_statistics(
        self, execution: TradeExecution, confirmation: TradeConfirmation
    ) -> None:
        ...
        if confirmation.fill_latency_ms:
            self.state.avg_fill_latency_ms = (self.state.avg_fill_latency_ms * 0.9) + (
        ...
        while self.is_running:
            try:
                ...
                self.state.is_healthy = (
                    self.state.kafka_connected and self.state.bybit_connected
                )
