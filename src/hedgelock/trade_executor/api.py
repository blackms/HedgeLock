...
trades_submitted = Counter(
    "trade_executor_trades_submitted_total", "Total number of trades submitted"
)
trades_filled = Counter(
    "trade_executor_trades_filled_total", "Total number of trades filled"
)
trades_failed = Counter(
    "trade_executor_trades_failed_total", "Total number of trades failed"
)
...
active_orders = Gauge("trade_executor_active_orders", "Number of active orders")
daily_volume = Gauge("trade_executor_daily_volume_btc", "Daily trading volume in BTC")
...
async def root() -> Dict[str, str]:
    """Root endpoint."""
    return {"service": "trade-executor", "version": "1.0.0", "status": "running"}
...
async def statistics() -> Dict[str, Any]:
    ...
    fill_rate = state.trades_filled / total_trades if total_trades > 0 else 0
...
if __name__ == "__main__":
    ...
    uvicorn.run(app, host="0.0.0.0", port=8004, log_level=config.log_level.lower())
