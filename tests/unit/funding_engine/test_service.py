...
class TestFundingEngineService:
    ...
    def service(self, config, mock_consumer, mock_producer, mock_storage):
        ...
        with patch('src.hedgelock.funding_engine.service.FundingEngineConsumer', return_value=mock_consumer), \
             patch('src.hedgelock.funding_engine.service.FundingEngineProducer', return_value=mock_producer), \
             patch('src.hedgelock.funding_engine.service.FundingStorage', return_value=mock_storage):
    ...
    async def test_handle_funding_rate_success(
        self, service, mock_storage, mock_producer,
        sample_funding_message, sample_funding_rate
    ):
    ...
    @pytest.mark.asyncio
    async def test_create_funding_snapshot_no_current_rate(
    ...
    @pytest.mark.asyncio
    async def test_periodic_context_update_error_handling(
        self, service, mock_storage
    ):
    ...
    @pytest.mark.asyncio
    async def test_process_funding_rates_error_recovery(
