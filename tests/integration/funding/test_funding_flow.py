...
from src.hedgelock.risk_engine.models import AccountData, RiskState, RiskStateMessage
...
class TestFundingFlow:
    ...
    def funding_rate_message(self):
        ...
        return FundingRateMessage(
            service="collector", funding_rate=rate, trace_id="test-funding-flow-123"
        )
    ...
    async def test_funding_rate_to_context_flow(self):
        ...
        assert (
            sent_msg.funding_context.current_regime == FundingRegime.NORMAL
        )  # 33% APR
        assert (
            sent_msg.funding_context.position_multiplier < 1.0
    ...
    async def test_funding_context_to_risk_state_flow(self):
        ...
        calculation = calculator.calculate_risk(
            account_data, funding_context, "test-123"
        )
