...
async def test_full_pipeline_caution_state(kafka_producer, kafka_consumer):
    ...
    assert (
        hedge_trade["hedge_decision"]["side"] == "Sell"
    )  # Should sell to reduce delta
    assert (
        abs(hedge_trade["hedge_decision"]["hedge_size"] - 0.48) < 0.01
    )  # Target delta 0.02
...
async def test_full_pipeline_critical_state(kafka_producer, kafka_consumer):
    ...
    assert (
        hedge_trade["hedge_decision"]["side"] == "Sell"
    )  # Should sell to reduce exposure
...
async def test_pipeline_error_handling():
    ...
    await producer.send(settings.kafka.topic_account_raw, value=malformed_data)
    await producer.flush()
    await producer.stop()
