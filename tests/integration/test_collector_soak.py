...
class CollectorSoakTest:
    ...
    async def consume_messages(self):
        ...
        async for message in self.consumer:
            ...
            self.messages_received.append(
                {"timestamp": timestamp, "source": source, "offset": message.offset}
            )

            # Check for data gaps
            if source in self.last_message_time:
                gap_seconds = (
                    timestamp - self.last_message_time[source]
                ).total_seconds()
    ...
    def analyze_results(self) -> Dict[str, Any]:
        ...
        # Find longest gap
        longest_gap = (
            max(self.gaps_detected, key=lambda x: x["gap_seconds"])
        ...
        return {
            ...
            "test_passed": all(
                g["gap_seconds"] <= self.max_allowed_gap_seconds
...
async def run_soak_test():
    ...
    try:
        ...
        ws_manager = WebSocketManager(
            kafka_producer=AsyncMock(), api_key="test_key", api_secret="test_secret"
        )
