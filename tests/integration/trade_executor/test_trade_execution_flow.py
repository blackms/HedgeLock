...
from src.hedgelock.hedger.models import (
    HedgeDecision, OrderSide, OrderType, OrderRequest, HedgeTradeMessage
)
...
class TestTradeExecutionFlow:
        ...
        while asyncio.get_event_loop().time() - start_time < timeout:
            try:
                ...
                records = await asyncio.wait_for(
                    kafka_consumer.getmany(timeout_ms=1000),
                    timeout=2
                )
    ...
    async def test_multiple_concurrent_trades(
        ...
        while len(confirmations) < num_trades and asyncio.get_event_loop().time() - start_time < 15:
            try:
                records = await asyncio.wait_for(
                    kafka_consumer.getmany(timeout_ms=1000),
                    timeout=2
